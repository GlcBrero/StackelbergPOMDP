...
"""Fail-closed smoke and preflight gates for the v4 Atari E1 seller.
...
from replication.atari import probe_atari_e1_seller_two_branch as probe
...
SCHEMA_VERSION = 1
GATE_KIND = "stackpomdp.atari.e1_seller_two_branch_diagnostics_gate.v4"
SMOKE_KIND = "stackpomdp.atari.e1_seller_two_branch_mechanics_smoke.v4"
PREFLIGHT_KIND = (
    "stackpomdp.atari.e1_seller_two_branch_conditioning_preflight.v4"
)
SOURCE_KIND = "seller_conditioning_recovery_v4_two_branch_v1"
ROLE = "seller"
ARCHITECTURE = "seller_two_branch_beta_v4"
...
TOKEN = "conditioning_recovery_v4_two_branch_v1"
...
WANDB_JOB_TYPE = (
    "atari_e1_seller_conditioning_recovery_v4_two_branch_v1_uniform_formal"
)
WANDB_NAME = (
    "atari_clean_e1_seller_conditioning_recovery_v4_two_branch_v1_"
...
def canonical_architecture():
    ...
    _require(
        value.get("schema")
        == "stackpomdp.atari.economic_actor_architecture.v4",
        "v4 architecture schema changed",
    )
    _require(
        value.get("parameterization") == ARCHITECTURE,
        "v4 architecture parameterization changed",
    )
    return value


def canonical_initialization():
    value = trainer.two_branch_initialization_contract()
    _require(
        value.get("schema")
        == "stackpomdp.atari.e1_two_branch_initialization.v1",
        "v4 initialization schema changed",
    )
    _require(
        value.get("architecture_parameterization") == ARCHITECTURE,
        "v4 initialization parameterization changed",
    )
    return value
...
def _git_revision(code_root):
    ...
    _require(
        ...
        "v4 code revision is not a full lowercase SHA",
    )
    return revision
...
def _git_scoped_clean(code_root):
    ...
    _require(
        ...
        "v4 validator is not executing from --code-root",
    )
    paths = (
    ...
    _require(not dirty, f"refusing dirty v4 seller code: {dirty}")
...
def _load_metadata(checkpoint, *, e0b, expected_revision):
    ...
    try:
        ...
        _require(
            ...
            "loaded v4 policy architecture changed",
        )
    finally:
        del model
    ...
    _require(
        ...
        "v4 checkpoint was trained by another code revision",
    )
    _require(
        metadata.get("e0b_source_provenance", {}).get("sha256")
        == sha256_file(e0b)
        == CANONICAL_E0B_SHA256,
        "v4 checkpoint does not use canonical E0b bytes",
    )
    return metadata
...
def _validate_checkpoint_metadata(
    ...
    _same_path(metadata.get("path"), checkpoint, label="v4 checkpoint")
    _require(
        metadata.get("training_timesteps") == timesteps,
        "v4 diagnostic clock changed",
    )
    _require(
        metadata.get("training_config") == canonical_training_config(),
        "v4 PPO/architecture configuration changed",
    )
    _require(
        metadata.get("economic_architecture") == canonical_architecture(),
        "v4 checkpoint architecture provenance changed",
    )
    _require(
        metadata.get("two_branch_initialization")
        == canonical_initialization(),
        "v4 checkpoint initialization provenance changed",
    )
    _require(
        ...
        "v4 diagnostic is not fresh independent Uniform(0,1)^5 training",
    )
    _require(
        metadata.get("resume_source") is None,
        "v4 diagnostic resumed another checkpoint",
    )
    _require(
        metadata.get("optimizer_adam_step") == timesteps // 820 * 4,
        "v4 diagnostic optimizer clock changed",
    )
    _require(
        metadata.get("e1_training_code_revision") == expected_revision,
        "v4 diagnostic revision changed",
    )
    ...
    _require(
        ...
        "v4 diagnostic gameplay actor was not frozen bit-exactly",
    )


def _validate_trace(path, *, checkpoint, timesteps):
    return shared._validate_training_trace(
        path,
        mode=SAMPLER_MODE,
        first_step=820,
        last_step=timesteps,
...
def _validate_evaluation(path, *, metadata, timesteps):
    ...
    _require(
        ...
        "v4 diagnostic evaluation architecture changed",
    )
    _require(
        provenance.get("two_branch_initialization")
        == canonical_initialization(),
        "v4 diagnostic evaluation initialization changed",
    )
    _require(
        provenance.get("e1_training_code_revision")
        == metadata.get("e1_training_code_revision"),
        "v4 diagnostic evaluation revision changed",
    )
    return record
...
def validate_smoke(
    ...
    checkpoint = Path(checkpoint).expanduser().resolve()
    _exact_name(checkpoint, SMOKE_CHECKPOINT_NAME, "v4 smoke checkpoint")
    _same_path(
        training_log,
        _expected_sidecar(checkpoint, "training.jsonl"),
        label="v4 smoke trace",
    )
    _same_path(
        evaluation,
        _expected_sidecar(checkpoint, "evaluation.json"),
        label="v4 smoke evaluation",
    )
    ...
    return {
        ...
        "two_branch_initialization": canonical_initialization(),
...
def _compare_probe(reported, inferred):
    ...
    _require(
        reported == inferred,
        "v4 preflight probe differs from fresh checkpoint inference",
    )
...
def _validate_conditioning_probe(
    ...
    _exact_name(path, PREFLIGHT_PROBE_NAME, "v4 preflight probe")
    value = load_json(path)
    _require(value.get("probe") == probe.PROBE_NAME, "unknown v4 probe")
    _require(
        value.get("execution") == {
        ...
        "v4 probe execution contract changed",
    )
    _require(
        value.get("metadata_verification", {}).get("passed") is True,
        "v4 probe metadata verification failed",
    )
    record = value.get("checkpoint", {})
    _same_path(record.get("path"), checkpoint, label="v4 probe checkpoint")
    _require(
        record.get("sha256") == sha256_file(checkpoint)
        and record.get("training_timesteps") == PREFLIGHT_TIMESTEPS,
        "v4 probe checkpoint identity changed",
    )
    _require(
        record.get("economic_architecture") == canonical_architecture()
        and record.get("two_branch_initialization")
        == canonical_initialization(),
        "v4 probe architecture/initialization changed",
    )
    _require(
        record.get("e1_training_code_revision") == expected_revision,
        "v4 probe revision changed",
    )
    expected_checks = {
    ...
    _require(
        ...
        "v4 conditioning preflight failed a preregistered gate",
    )
    ...
    for episode in result.get("episode_rows", []):
        for event in episode.get("events", []):
            rows.append({
                ...
                "v4_context_ablation": bool(context_ablation),
                **event,
            })
...
def _validate_behavior_result(
    ...
    _require(
        ...
        f"v4 behavioral {phase} seed schedule changed",
    )
    for index, (row, context) in enumerate(zip(rows, contexts)):
        _require(
            ...
            f"v4 behavioral {phase} episode identity changed",
        )
        _same_path(
            row.get("checkpoint_path"),
            metadata["path"],
            label=f"v4 behavioral {phase} checkpoint",
        )
        _require(
            ...
            f"v4 behavioral {phase} context {index} changed",
        )
        _require(
            bool(row.get("v4_context_ablation", False))
            is bool(context_ablation),
            f"v4 behavioral {phase} ablation flag changed",
        )
        ...
        _require(
            ...
            f"v4 behavioral {phase} seeded schedule {index} changed",
        )
    violations = [
        violation
        for row in rows
    ...
    _require(
        ...
        f"v4 behavioral {phase} mechanics failed: {violations[:3]}",
    )
    _require(
        result.get("summary") == evaluator._summary(rows, role=ROLE),
        f"v4 behavioral {phase} summary does not recompute",
    )
    _require(
        result.get("event_rows") == _expected_event_rows(
            result, phase=phase, context_ablation=context_ablation
        ),
        f"v4 behavioral {phase} event rows do not recompute",
    )
    _require(
        len(result.get("event_rows", [])) == 5 * len(rows),
        f"v4 behavioral {phase} does not retain all five events",
    )
...
def run_behavioral_preflight(
    ...
    _exact_name(
        ...
        "v4 behavioral-preflight checkpoint",
    )
    _exact_name(
        output, PREFLIGHT_BEHAVIOR_NAME,
        "v4 behavioral-preflight report",
    )
    _require(
        not os.path.lexists(output),
        "refusing to overwrite immutable v4 behavioral preflight",
    )
    _require(
        sha256_file(e0b) == CANONICAL_E0B_SHA256,
        "v4 behavioral preflight requires canonical E0b",
    )
    _require(
        sha256_file(rom) == CANONICAL_ROM_SHA256,
        "v4 behavioral preflight requires canonical ROM",
    )
    metadata = _load_metadata(
        checkpoint, e0b=e0b, expected_revision=expected_revision
    )
    ...
    try:
        _require(
            ...
            "v4 behavioral preflight loaded different checkpoint bytes",
        )
    ...
    _require(
        ...
        "v4 behavioral preflight changed checkpoint bytes",
    )
    ...
    report = {
        "schema_version": SCHEMA_VERSION,
        "kind": (
            "stackpomdp.atari.e1_seller_two_branch_behavioral_preflight.v4"
        ),
...
def validate_behavioral_preflight(
    ...
    _require(
        ...
        "unknown v4 behavioral-preflight report",
    )
    _require(
        sha256_file(e0b) == CANONICAL_E0B_SHA256
        and value.get("e0b", {}).get("sha256") == CANONICAL_E0B_SHA256,
        "v4 behavioral-preflight E0b changed",
    )
    _require(
        sha256_file(rom) == CANONICAL_ROM_SHA256
        and value.get("rom", {}).get("sha256") == CANONICAL_ROM_SHA256,
        "v4 behavioral-preflight ROM changed",
    )
    ...
    _require(
        ...
        "v4 behavioral-preflight checkpoint metadata changed",
    )
    args = _behavior_args(e0b=e0b, rom=rom)
    _require(
        value.get("environment") == evaluator.environment_config(args),
        "v4 behavioral-preflight environment changed",
    )
    _require(
        value.get("protocol") == {
            "random_episodes": PREFLIGHT_BEHAVIOR_EPISODES,
        ...
        "v4 behavioral-preflight protocol changed",
    )
    random_seeds = list(range(
        PREFLIGHT_BEHAVIOR_SEED_START,
        PREFLIGHT_BEHAVIOR_SEED_START + PREFLIGHT_BEHAVIOR_EPISODES,
    ...
    _require(
        ...
        "v4 full and joint-ablation schedules are not paired",
    )
    fixed = value.get("fixed_contexts", [])
    _require(
        len(fixed) == len(PREFLIGHT_FIXED_VALUES),
        "v4 behavioral fixed grid is incomplete",
    )
    fixed_seeds = list(range(
        PREFLIGHT_FIXED_SEED_START,
        PREFLIGHT_FIXED_SEED_START + PREFLIGHT_BEHAVIOR_EPISODES,
    ...
    for result, fixed_value in zip(fixed, PREFLIGHT_FIXED_VALUES):
        _require(
            ...
            "v4 behavioral fixed-grid value changed",
        )
    ...
    _require(
        ...
        "v4 behavioral gate does not recompute from episode/event rows",
    )
    passed = bool(gate.get("passed") is True)
    _require(
        value.get("passed") is passed,
        "v4 behavioral-preflight outcome changed",
    )
    if require_pass:
        _require(passed, "v4 82k real-ALE behavioral preflight failed")
    return {
        "path": str(path),
        "sha256": sha256_file(path),
        "passed": passed,
...
def validate_preflight(
    ...
    checkpoint = Path(checkpoint).expanduser().resolve()
    _exact_name(
        checkpoint, PREFLIGHT_CHECKPOINT_NAME, "v4 preflight checkpoint"
    )
    _same_path(
        training_log,
        _expected_sidecar(checkpoint, "training.jsonl"),
        label="v4 preflight trace",
    )
    _same_path(
        evaluation,
        _expected_sidecar(checkpoint, "evaluation.json"),
        label="v4 preflight evaluation",
    )
    metadata = _load_metadata(
        checkpoint, e0b=e0b, expected_revision=expected_revision
    )
...
def build_gate(args):
    ...
    _exact_name(output, GATE_NAME, "v4 diagnostics gate")
    e0b = Path(args.e0b).expanduser().resolve()
    rom = Path(args.rom).expanduser().resolve()
    seller_release = Path(args.seller_release).expanduser().resolve()
    _require(
        sha256_file(e0b) == CANONICAL_E0B_SHA256,
        "v4 gate requires exact canonical E0b bytes",
    )
    _require(
        sha256_file(rom) == CANONICAL_ROM_SHA256,
        "v4 gate requires exact canonical ROM bytes",
    )
    shared.validate_seller_release(seller_release)
    _require(
        sha256_file(seller_release) == CANONICAL_SELLER_RELEASE_SHA256,
        "v4 gate requires exact canonical buyer-release bytes",
    )
    formal = Path(args.formal_checkpoint).expanduser().resolve()
    _exact_name(formal, FORMAL_CHECKPOINT_NAME, "v4 formal checkpoint")
    _require(
        not os.path.lexists(formal),
        "formal v4 checkpoint exists before its diagnostics gate",
    )
    ...
    _require(
        ...
        "v4 smoke and conditioning preflight are not distinct runs",
    )
    _git_scoped_clean(args.code_root)
    _require(
        _git_revision(args.code_root) == revision,
        "v4 code revision changed while building diagnostics gate",
    )
    return {
        "schema_version": SCHEMA_VERSION,
        "kind": GATE_KIND,
...
def _validate_prerequisites(value):
    ...
    for key, digest in expected.items():
        ...
        _require(
            ...
            == record.get("sha256")
            == digest,
            f"v4 gate prerequisite changed: {key}",
        )
...
def validate_gate(path, *, code_root=None):
    ...
    _require(smoke_record == smoke, "v4 smoke evidence changed")
    ...
    _require(
        ...
        "v4 conditioning-preflight evidence changed",
    )
    formal = value.get("formal_release", {})
    _exact_name(
        formal.get("checkpoint", ""),
        FORMAL_CHECKPOINT_NAME,
        "v4 formal checkpoint",
    )
    _require(
        formal == {
            "allowed": True,
        ...
        "v4 formal-release protocol changed",
    )
    if code_root is not None:
        _require(
            _git_revision(code_root) == revision,
            "runtime revision differs from v4 diagnostics gate",
        )
...
def write_or_validate_gate(args):
    ...
    _require(
        ...
        "refusing symlink v4 diagnostics-gate output",
    )
