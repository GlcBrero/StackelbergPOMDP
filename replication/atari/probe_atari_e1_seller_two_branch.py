"""No-ALE learned-conditioning probe for the seller two-branch v4 policy.
...
ablations of the context branch and event-specific current-threshold skip.
...
PROBE_NAME = "clean_atari_e1_seller_two_branch_probe_v4"
PROBE_GATE_NAME = "clean_e1_seller_two_branch_conditioning_gate_v4"
ARCHITECTURE_SCHEMA = "stackpomdp.atari.economic_actor_architecture.v4"
...
def canonical_initialization_provenance(policy):
    return trainer.two_branch_initialization_provenance(policy)


def validate_loaded_seller(model, metadata, e0b_metadata):
    """Require the exact v4 architecture and frozen E0b gameplay actor."""
    ...
    checks.update({
        ...
        "policy_initialization_provenance_exact": (
            initialization == trainer.two_branch_initialization_contract()
        ),
        "model_initialization_provenance_exact": (
            recorded_initialization == initialization
        ),
        "metadata_initialization_provenance_exact": (
            metadata.get("two_branch_initialization") == initialization
        ),
        "training_config_enables_only_v4": (
        ...
        "context_branch_shapes_exact": (
            policy.economic_context_encoder[0].in_features == 5
            and policy.economic_context_encoder[0].out_features == 32
            and policy.economic_context_output.in_features == 32
            and policy.economic_context_output.out_features == 5
            and tuple(policy.economic_current_slopes.shape) == (5,)
        ),
    ...
    if failed:
        raise ValueError(
            "seller checkpoint failed two-branch probe checks: "
            + ", ".join(failed)
        )
    return {
        "passed": True,
        ...
        "two_branch_initialization": initialization,
...
def economic_beta_mean(model, values):
    ...
    if means.shape != (1,):
        raise RuntimeError("two-branch probe expected one Beta distribution")
...
def _snapshot_ablatable_parameters(policy):
    return {
        ...
        "current_slopes": policy.economic_current_slopes.detach().clone(),
    }
...
def _snapshot_equal(policy, snapshot):
    return bool(
        ...
        and th.equal(
            policy.economic_current_slopes.detach(),
            snapshot["current_slopes"],
        )
    )
...
def _in_memory_ablation(policy, *, context_branch, current_skip):
    """Temporarily zero selected v4 paths and restore them bit-exactly."""
    ...
    try:
        ...
        with th.no_grad():
            ...
            policy.economic_current_slopes.copy_(snapshot["current_slopes"])
        if not _snapshot_equal(policy, snapshot):
            raise RuntimeError("in-memory two-branch ablation did not restore")
...
def _variant_ablation(variant):
    ...
    if variant == VARIANT_BOTH_ABLATED:
        return True, True
    raise ValueError(f"unknown two-branch probe variant: {variant}")
...
def two_branch_conditioning_gate(variants, learning_evidence):
    """Apply immutable learned-response and reversible-ablation gates."""

    if set(variants) != set(VARIANTS):
        raise ValueError("two-branch probe variants changed")
    ...
    return {
        ...
        "diagnostics_not_gated": {
            ...
            "reason_not_gated": (
                ...
                "event-specific current-threshold skip"
            ),
        },
    }
...
def conditioning_learning_evidence(policy):
    ...
    named = {
        ...
        "current_slopes": policy.economic_current_slopes,
    }
    linf = {
...
def collect_conditioning_report(model):
    ...
    if not _snapshot_equal(policy, baseline):
        raise RuntimeError("two-branch probe changed policy parameters")
    ...
    return {
        ...
        "warmup_gate": two_branch_conditioning_gate(
...
def run_probe_from_checkpoints(*, checkpoint, e0b_checkpoint, device="cpu"):
    ...
    if after != before or metadata["sha256"] != before:
        raise RuntimeError("seller checkpoint bytes changed during v4 probe")
    return {
        "probe": PROBE_NAME,
...
def main(argv=None):
    ...
    if args.output:
        ...
        if output.exists() or output.is_symlink():
            raise FileExistsError(f"refusing to overwrite v4 probe: {output}")
