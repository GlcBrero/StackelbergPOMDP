"""Trade schedules and opponent-commitment sampling.

Sampling is part of the scientific environment definition: it runs at every
episode reset and is therefore library code rather than an offline tool.
"""
...
class ExactFiveEventSchedule:
    """Sample five sorted, distinct event times from a gameplay window."""
    ...
    def __init__(
        ...
        if self.gameplay_horizon < NUM_TRADE_EVENTS:
            raise ValueError("gameplay_horizon must contain at least five steps")
        if not 0 <= self.tail_steps < self.gameplay_horizon:
            raise ValueError("tail_steps must lie in [0, gameplay_horizon)")
    ...
    def _validate_fixed(self, values):
        ...
        if fixed[0] < 0 or fixed[-1] >= self.event_stop:
            raise ValueError("fixed_event_steps must fit in the event window")
        return fixed
