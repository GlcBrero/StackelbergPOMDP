"""
Verify Q-learning collusion à la Calvano et al. (2019).
Runs Q-learners through BertrandCompetitionEnv with no platform intervention.
Only m (price grid size) varies: m=15 for Calvano replication, m=5 for our experiments.

Saves converged Q-tables to --output_dir for use by calvano_deviation.py.
"""
...
import numpy as np
from stackelberg_pomdp.calvano import (
    DEFAULT_CALVANO,
    average_symmetric_gain_index,
    symmetric_profit,
)

ALPHA = DEFAULT_CALVANO.alpha
BETA = DEFAULT_CALVANO.beta
DELTA = DEFAULT_CALVANO.discount
CONVERGENCE_WINDOW = DEFAULT_CALVANO.convergence_window
MAX_STEPS = DEFAULT_CALVANO.max_steps


def run_session(m, seed, alpha=None, beta=None, platform_intervention='no_intervene'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default=None,
                        help='Directory to save converged Q-tables. Default: results/calvano_m{m}/')
    args = parser.parse_args()

    output_dir = args.output_dir or os.path.join(os.path.dirname(__file__), f'results/calvano_m{args.m}')
    os.makedirs(output_dir, exist_ok=True)

    from stackelberg_pomdp.gym_envs.envs.custom_envs import BertrandCompetitionEnv
    ...
    print(f"Calvano replication: m={args.m}, alpha={args.alpha}, beta={args.beta}, intervention={args.platform_intervention}")
    ...
    for i in range(args.n_sessions):
        ...
        state = run_session(args.m, seed, alpha=args.alpha, beta=args.beta, platform_intervention=args.platform_intervention)
    ...
    print(f"Calvano expects Delta ~ 0.7-0.9")
